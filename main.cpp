...
Point2d Interpolate(double t, int degree, vector<Point2d> points, vector<int> knots) {
	...
	for (l = 1; l <= degree + 1; l++) {
		for (i = s; i > s - degree - 1 + l; i--) {
			...
			//tx = (1 - alpha) * points[i - 1].x + alpha * points[i].x;
			//ty = (1 - alpha) * points[i - 1].y + alpha * points[i].y;
			////cout << tx << "  " << ty << endl;

			//pt.push_back(Point2d(tx, ty));
			
			//circle(output, Point2d(tx,ty) * 100 + Point2d(200, 200), 1, Scalar(0, 255, 0), -1);
		}
	}
	tx = v[s][0] ;
	ty = v[s][1];
